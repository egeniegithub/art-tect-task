  ...
  await fetch(`https://api.artic.edu/api/v1/exhibitions`)
    ...
    .then((json) => {
      if (json.status >= 400 && json.status < 405) {
        ...
        console.log("the json coming is", json);
...
const Home = (props: homeProps) => {
  ...
  console.log("the home pagfe is", props);
  useEffect(() => {
    setArtData(props?.data)
    setPaginationData(props?.pagination)
  }, [])
  const fetchData = async () => {
    console.log("paginationData?.next_url", paginationData?.next_url);
  ...
  console.log("the pagination rtotal is - - - - -- ", paginationData);
  console.log("the art data rtotal is = = = = = = = == =  ", artData);
  ...
  return (
    <>
      ...
      <InfiniteScroll
        ...
        next={async () => {
          console.log("i am in the fetch data");
          await fetchData();
        }}
        hasMore={true}
